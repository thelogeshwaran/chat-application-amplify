  ...
  function subscriptioned(id) {
    roomName = API.graphql(
      ...
      next: ({ value }) => {
        ...
        } else if(room.id === value.data.onCreateMessage.messageConversationId ) { 
  ...
  return (
    <div className="flex justify-center  min-h-1/10 text-xl">
      ...
      <Link
        ...
        className={(room.id === conversationId ? "bg-chatBlue flex items-center justify-between w-11/12 p-4 rounded-xl my-3" : "bg-chatPurpleDark justify-between flex items-center w-11/12 p-4 rounded-xl my-3" )}
      >
        <div className="flex">
        <img
          alt="img"
